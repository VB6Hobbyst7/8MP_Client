...
Public Class clsMPFlex
    ...
    'To support Decode/Encode Json by using Script control (java)
    Private ScriptEngine As MSScriptControl.ScriptControlClass
    ...
    Public Property cacheUrl As String
        Get
            ...
            Return vCacheUrl
        End Get
        Set(ByVal vCacheUrl_ As String)
            ' Sets the property value.
            vCacheUrl = vCacheUrl_
        End Set
    End Property
    '--------------------
    ...
    Public Property code_name As String
        Get
            ...
            Return vCodeName
        End Get
        Set(ByVal vCodeName_ As String)
            ' Sets the property value.
            vCodeName = vCodeName_
        End Set
    End Property
    ...
    Public Property selectedValue As String
        ...
        End Get
        Set(ByVal selectedValue_ As String)
            ' Sets the property value.
            vSelectedValue = selectedValue_
        End Set
    End Property

    Dim vSelectedKey As String
    Public Property selectedKey As String
        Get
            ' Gets the property value.
            Return vSelectedKey
        End Get
        Set(ByVal selectedKey_ As String)
            ' Sets the property value.
            vSelectedKey = selectedKey_
        End Set
    End Property
    ...
    Private Sub InitScriptEngine()
        ScriptEngine = New MSScriptControl.ScriptControlClass
...
HasError:
        ...
        vLocalSuccess = False
        vLocalErrorMsg = "Error line:" & vScriptControl.Error.Line.ToString & vbCrLf &
                        Err.Description
        vLocalErrorMsg = "Compile Error : " & vCodeName & Environment.NewLine &
            vScriptControl.Error.Description & Environment.NewLine &
            "Line  " & vScriptControl.Error.Line & " : " & vScriptControl.Error.Source
        destroyObject()
    End Function
    ...
    Private Sub initialScript()
        ...
        'Added on Dec 29,2019 -- To support API service
        Dim ApiService As New clsAPIService
        With ApiService
            .Url = vLocalUrl
        ...
        With vScriptControl
            ...
            'API Objects
            .AddObject("api", ApiService, True)

            'Json encoder/decode
            .AddObject("json", ScriptEngine, True)
            ...
            .AddObject("cache_url", vCacheUrl, True)
            .AddObject("message", vLocalMsg, True)
        End With

        'Initial GLOBAL Variable
        vScriptControl.AddCode("SYSTEM_URL = """ & vLocalUrl & """")
        'vScriptControl.AddCode("CACHE_URL = """ & vCacheUrl & """")

        'Initial for Parameter variable and Value
        vScriptControl.AddCode("SELECTED_KEY = """ & vSelectedKey & """")
        vScriptControl.AddCode("SELECTED_VALUE = """ & vSelectedValue & """")
        ...
        Dim vGetObjectBySlugStr As String = ""
        vGetObjectBySlugStr = "Function getObjectBySlug(vModel_,vSlug_) " & vbCrLf &
                " dim outputx " & vbCrLf &
                " outputx = api.getJsonBySlug(vModel_,vSlug_) " & vbCrLf &
                " set getObjectBySlug = outputx " & vbCrLf &
                "End Function"
        vScriptControl.AddCode(vGetObjectBySlugStr)

        Dim vGetObjectByUrlStr As String = ""
        vGetObjectByUrlStr = "Function getObjectByUrl(vUrl_) " & vbCrLf &
                " dim outputx " & vbCrLf &
                " outputx = api.getJsonByUrl(vUrl_) " & vbCrLf &
                " getObjectByUrl = outputx " & vbCrLf &
                "End Function"
        vScriptControl.AddCode(vGetObjectByUrlStr)


        'JSON functions
        Dim vJsonEncodeStr As String = ""
        vJsonEncodeStr = "Function DecodeJson(JSonString) " & vbCrLf &
                        "Set DecodeJson = json.Eval(""("" & JSonString & "")"") " & vbCrLf &
                        "End Function"
        vScriptControl.AddCode(vJsonEncodeStr)

        Dim vJsonGetPropertyStr As String = ""
        vJsonGetPropertyStr = "Function GetJsonKey(JsonObject,propertyName) " & vbCrLf &
                            "GetJsonKey = json.Run(""getProperty"", JsonObject, propertyName)" & vbCrLf &
                            "End Function"
        vScriptControl.AddCode(vJsonGetPropertyStr)


        '--Get Cache Parameter
        Dim strGetParameter As String
        strGetParameter = "Function getCache(vParameter_) " & vbCrLf &
                " getCache = getCacheParameterByURL(cache_url,vParameter_) " & vbCrLf &
                "End Function"
        vScriptControl.AddCode(strGetParameter)
        '------------------------

        '--Set Cache Parameter
        Dim strSetParameter As String
        strSetParameter = "Function setCache(vParameter_,vValue_) " & vbCrLf &
                " setCache = setCacheParameterByURL(cache_url,vParameter_,vValue_) " & vbCrLf &
                "End Function"
        vScriptControl.AddCode(strSetParameter)
        '------------------------
        ...
        '        "End Function"
        vInclude = "Function includeSnippet(vSnippet_)" & vbCrLf &
                " vSnippetJsonStr = api.getJsonBySlug(""snippet"",vSnippet_)" & vbCrLf &
                " dim objSnippet " & vbCrLf &
                " Set objSnippet = json.Eval(""("" & vSnippetJsonStr & "")"") " & vbCrLf &
                " vCode = json.Run(""getProperty"", objSnippet, ""code"")" & vbCrLf &
                " clsMain.AddCode vCode  " & vbCrLf &
                "End Function"
...
HasError:
    ...
    Public Function getObject(vControlName As String) As Object  'vControlName=1.text1 (StepName.Control)
        ...
        If vSteps.Length = 1 Then 'One level
        ...
        'UBound(vSteps) = 1
        If vSteps.Length = 2 Then 'One level
            If Not Form Is Nothing Then
        ...
        If vSteps.Length = 3 Then 'One level'Two level
            If Not Form Is Nothing Then
...
HasError:
    ...
    Public Function getSnippetCode(ByVal _vSnippetName As String) As Object
        'Support request with Token
        Dim request As WebRequest = WebRequest.Create(vLocalUrl + "/api/snippet/" + _vSnippetName)
        request.Method = "GET"
    ...
    Public Function getCacheParameterByURL(_vCacheUrl As String, ByVal _parameter As String) As String
        Try
            Dim client As WebClient = New WebClient()
            Dim response As String = client.DownloadString(_vCacheUrl & "/" & _parameter)
            getCacheParameterByURL = response
    ...
    Public Function setCacheParameterByURL(_vCacheUrl As String, _parameter As String, _value As String) As String
        Try
            Dim client As WebClient = New WebClient()
            Dim reqparm As New Specialized.NameValueCollection
            Dim responsebytes = client.UploadValues(_vCacheUrl & "/" & _parameter & "/" & _value, "POST", reqparm)
            Dim responsebody = (New Text.UTF8Encoding).GetString(responsebytes)
            setCacheParameterByURL = responsebody
        Catch ex As Exception
            Return ""
        End Try
